  ...
  return (
    <Dialog open={open} onOpenChange={(open) => !open && onClose()}>
      <DialogContent 
        ...
        className="bg-card text-card-foreground border border-border rounded-xl shadow-2xl w-full max-w-lg p-0 [&>button]:hidden"
      >
        <DialogHeader className="px-6 py-5 border-b border-border flex flex-row items-center justify-between space-y-0 text-left">
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="contents">
            <div className="px-6 py-5 space-y-4">
              <FormField
                ...
                render={({ field }) => (
                  <FormItem className="space-y-0">
                    <FormLabel className="text-[0.65rem] font-semibold tracking-[0.15em] uppercase text-muted-foreground mb-1.5 block">Supplier Name *</FormLabel>
                    <FormControl>
                      <Input 
                        placeholder="Enter supplier name" 
                        className="bg-background border border-border rounded-md text-sm text-foreground h-9 px-3 placeholder:text-muted-foreground focus:outline-none focus:ring-1 focus:ring-[#E8400C] focus:border-[#E8400C] focus-visible:ring-0 focus-visible:ring-offset-0" 
                        {...field} 
                      />
                    </FormControl>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem className="space-y-0">
                    <FormLabel className="text-[0.65rem] font-semibold tracking-[0.15em] uppercase text-muted-foreground mb-1.5 block">Payment Terms</FormLabel>
                    <FormControl>
                      <Textarea
                        placeholder="e.g., Net 30, COD, 2/10 Net 30"
                        className="bg-background border border-border rounded-md text-sm text-foreground min-h-[80px] px-3 py-2 placeholder:text-muted-foreground focus:outline-none focus:ring-1 focus:ring-[#E8400C] focus:border-[#E8400C] resize-none focus-visible:ring-0 focus-visible:ring-offset-0"
                        {...field}
                      />
                    </FormControl>
                    <FormMessage className="text-xs text-red-500 mt-1" />
            ...
            <DialogFooter className="px-6 py-4 border-t border-border flex flex-row gap-3 justify-end sm:justify-end space-x-0">
              <Button 
                type="button" 
                variant="outline" 
                onClick={onClose} 
                disabled={isSubmitting}
                className="border border-border text-foreground hover:bg-accent rounded-md h-9 px-5 text-xs font-semibold uppercase tracking-[0.12em] bg-transparent"
              >
                Cancel
              </Button>
              <Button 
                type="submit" 
                disabled={isSubmitting}
                className="bg-[#E8400C] text-white hover:bg-[#c73508] rounded-md h-9 px-5 text-xs font-semibold uppercase tracking-[0.12em] border-none"
              >
