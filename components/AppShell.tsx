...
function Sidebar({
  ...
  return (
    <motion.aside
      ...
      className={cn(
        "bg-sidebar border-r border-sidebar-border h-full flex flex-col relative z-40 transition-shadow duration-300 shrink-0",
        !isExpanded && "shadow-[4px_0_24px_rgba(0,0,0,0.1)]"
      )}
    >
      {/* Logo area */}
      <div className="px-5 py-5 border-b border-sidebar-border flex items-center justify-between min-h-[73px]">
          ...
          <AnimatePresence mode="wait">
            {isExpanded && (
              <motion.span
                ...
                exit={{ opacity: 0, x: -10 }}
                className="font-sans text-lg font-bold text-sidebar-foreground whitespace-nowrap tracking-tight"
              >
                VendoFlow
              </motion.span>
            )}
      ...
      <button
        ...
        className="absolute -right-3.5 top-1/2 -translate-y-1/2 w-7 h-10 bg-sidebar border-y border-r border-sidebar-border rounded-r-md flex items-center justify-center text-muted-foreground hover:text-sidebar-foreground transition-colors z-50 group/toggle"
      >
        {isExpanded ? (
      ...
      <nav className="flex-1 px-3 py-4 space-y-1 overflow-y-auto overflow-x-hidden">
        {visibleNavItems.map(({ href, label, icon: Icon }) => (
          <Link
            ...
            href={href}
            className={cn(
              "group relative flex items-center gap-3 py-2.5 text-xs font-medium tracking-wide transition-all duration-150 rounded-md mx-2",
              isExpanded ? "px-4" : "px-0 justify-center",
              isActive(href)
                ? "bg-sidebar-accent text-sidebar-accent-foreground border-l-2 border-[#E8400C] font-semibold shadow-xs"
                : "text-muted-foreground hover:text-sidebar-foreground hover:bg-sidebar-accent/50"
            )}
          >
            <Icon className={cn("w-4 h-4 shrink-0 transition-transform duration-200", !isExpanded && "group-hover:scale-110", isActive(href) && "text-[#E8400C]")} />
            ...
            <AnimatePresence mode="wait">
              {isExpanded ? (
                ...
                <span className="absolute left-10 invisible group-hover:visible opacity-0 group-hover:opacity-100 transition-all duration-200 bg-popover text-popover-foreground text-[0.65rem] py-1 px-3 rounded shadow-xl border border-border whitespace-nowrap z-[100] translate-x-1 group-hover:translate-x-0">
                  {label}
                </span>
              )}
            </AnimatePresence>
      ...
      <div className="mt-auto px-5 py-4 border-t border-sidebar-border min-h-[69px] overflow-hidden space-y-3">
        {/* Theme Toggle */}
        <button
          onClick={() => setTheme(theme === "dark" ? "light" : "dark")}
          className={cn(
            "flex items-center gap-3 py-2 text-xs font-medium text-muted-foreground hover:text-sidebar-foreground rounded-md transition-colors w-full",
        ...
        <div className="flex items-center gap-3 pt-1 border-t border-sidebar-border/50">
          <div className="w-7 h-7 shrink-0 rounded-sm bg-sidebar-accent border border-sidebar-border flex items-center justify-center">
          ...
          <AnimatePresence mode="wait">
            {isExpanded && (
              ...
              >
                ...
                <p className="text-[0.65rem] tracking-[0.1em] uppercase text-muted-foreground truncate">{userRoleLabel}</p>
                <button 
                  onClick={handleSignOut}
                  className="flex items-center gap-3 px-0 py-1.5 w-full text-[0.65rem] font-semibold tracking-[0.1em] uppercase text-muted-foreground hover:text-destructive hover:bg-destructive/5 rounded-sm transition-colors mt-1"
                >
