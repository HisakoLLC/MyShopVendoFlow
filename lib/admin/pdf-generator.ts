...
export async function generateAndUploadReportPDF(
  ...
  const formatReportPeriod = (type: string, start: Date, end: Date): string => {
    ...
    if (type === 'daily') return `${start.getUTCDate()} ${months[start.getUTCMonth()]} ${start.getUTCFullYear()}`
    if (type === 'monthly') return `${months[start.getUTCMonth()]} ${start.getUTCFullYear()}`
  ...
  doc.text(`Generated by VendoFlow · ${utcTime}`, margin + 12, footerY + 1)
